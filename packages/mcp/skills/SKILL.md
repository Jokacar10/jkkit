...
3. Call `get_transaction_status` with the `normalizedHash` from the send response to verify the transaction actually completed on-chain

### Send Token
1. Call `get_jettons` to find token address and verify balance
2. Call `send_jetton` with token address and amount
3. Call `get_transaction_status` with the `normalizedHash` from the send response to verify the transaction actually completed
...
5. Call `get_transaction_status` with the `normalizedHash` from the response to verify the swap completed
...
- **After sending any transaction**, call `get_transaction_status` with the returned `normalizedHash` to confirm it actually completed on-chain—the send response alone does not guarantee finality
