...
import type { Hex, UserFriendlyAddress, Base64String } from '../models/core/Primitives';
...
export class Wallet {
     ...
     */
    async createTransferTonTransaction(request: TONTransferRequest): Promise<TransactionRequest> {
        const internalParams = this.mapTonTransferRequest(request);
        const result = await this.internalWallet.createTransferTonTransaction(internalParams);
     ...
     */
    async createTransferMultiTonTransaction(requests: TONTransferRequest[]): Promise<TransactionRequest> {
        const internalParams = {
            messages: requests.map((r) => this.mapTonTransferRequest(r)),
        };
        const result = await this.internalWallet.createTransferMultiTonTransaction(internalParams);
        return this.transactionRequestMapper.reverse(result);
    }
     ...
     */
    async sendTransaction(request: TransactionRequest): Promise<TransactionApprovalResponse> {
     ...
     */
    async createTransferJettonTransaction(request: JettonsTransferRequest): Promise<TransactionRequest> {
        const internalParams = this.mapJettonTransferRequest(request);
        const result = await this.internalWallet.createTransferJettonTransaction(internalParams);
        return this.transactionRequestMapper.reverse(result);
    }
     ...
     */
    async createTransferNftTransaction(request: NFTTransferRequest): Promise<TransactionRequest> {
        const internalParams = this.mapNftTransferRequest(request);
        const result = await this.internalWallet.createTransferNftTransaction(internalParams);
        return this.transactionRequestMapper.reverse(result);
    }
     ...
     */
    private mapTonTransferRequest(request: TONTransferRequest): TonTransferParams {
        const base = {
            toAddress: request.recipientAddress,
            amount: request.transferAmount,
            mode: request.mode ? (this.sendModeMapper.reverse(request.mode) as SendMode) : undefined,
     ...
     */
    private mapJettonTransferRequest(request: JettonsTransferRequest): JettonTransferParams {
        return {
            toAddress: request.recipientAddress,
     ...
     */
    private mapNftTransferRequest(request: NFTTransferRequest): NftTransferParamsHuman {
        return {
