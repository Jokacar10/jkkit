...
import type { TransactionMoneyFlow, TransactionMoneyFlowItem } from '../models/transactions/TransactionMoneyFlow';
 ...
 */
export class TransactionMoneyFlowMapper extends Mapper<MoneyFlow, TransactionMoneyFlow> {
    map(input: MoneyFlow): TransactionMoneyFlow {
        ...
        for (const transfer of input.allJettonTransfers) {
            const item: TransactionMoneyFlowItem = {
                assetType: transfer.type === 'jetton' ? AssetType.jetton : AssetType.ton,
