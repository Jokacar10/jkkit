...
import type { TransactionRequest, TransactionRequestMessage } from '../models/transactions/TransactionRequest';
 ...
 */
export class TransactionRequestMapper extends Mapper<TransactionRequest, ConnectTransactionParamContent> {
    private networkMapper = new NetworkMapper();
    private sendModeMapper = new SendModeMapper();

    private mapExtraCurrency(input: Record<string, string> | undefined): ConnectExtraCurrency | undefined {
    ...
    map(input: TransactionRequest): ConnectTransactionParamContent {
        const messages: ConnectTransactionParamMessage[] = input.messages.map((msg: TransactionRequestMessage) => ({
        ...
        return {
            messages,
            network: input.network ? this.networkMapper.reverse(input.network) : undefined,
    ...
    reverse(input: ConnectTransactionParamContent): TransactionRequest {
        const messages: TransactionRequestMessage[] = input.messages.map((msg: ConnectTransactionParamMessage) => ({
        ...
        return {
            messages,
            network: input.network ? this.networkMapper.map(input.network as CHAIN) : undefined,
            validUntil: input.valid_until,
            fromAddress: input.from,
        };
    }

    private reverseExtraCurrency(input: ConnectExtraCurrency | undefined): Record<string, string> | undefined {
        if (!input) return undefined;
