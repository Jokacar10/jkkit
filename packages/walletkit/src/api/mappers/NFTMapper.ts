...
export class NFTMapper extends Mapper<NftItem, NFT> {
    ...
    map(input: NftItem): NFT {
        return {
            ...
            index: input.index,
            info: input.metadata ? this.tokenInfoMapper.mapFromNft(input.metadata) : undefined,
            attributes: input.attributes?.map((attr) => this.attributeMapper.map(attr)),
            collection: input.collection ? this.collectionMapper.map(input.collection) : undefined,
            ...
            ownerAddress: input.ownerAddress ? this.addressMapper.map(input.ownerAddress) : undefined,
            realOwnerAddress: input.realOwner ? this.addressMapper.map(input.realOwner) : undefined,
