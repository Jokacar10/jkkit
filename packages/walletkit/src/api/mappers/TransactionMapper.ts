...
import type { ToncenterTransaction, EmulationAccountStatus } from '../../types/toncenter/emulation';
import type { Transaction, AccountStatus } from '../models/transactions/Transaction';
import { Mapper } from './Mapper';

/**
 * Maps ToncenterTransaction to API Transaction model.
 */
export class TransactionMapper extends Mapper<ToncenterTransaction, Transaction> {
    private mapAccountStatus(status: EmulationAccountStatus | string): AccountStatus {
        switch (status) {
            case 'active':
                return { type: 'active' };
            case 'frozen':
